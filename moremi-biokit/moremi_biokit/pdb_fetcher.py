...
from .connectors.rcsb import download_pdb_from_rcsb
from .connectors.uniprot import download_pdb_from_uniprot
from .connectors.ncbi import download_pdb_from_ncbi

# Import for structure prediction
from .proteins.analysis_tools import structure_predictor

# Define the package path for internal PDBs
_INTERNAL_PDB_BASE_PKG_PATH = "moremi_biokit.test_pdb_targets" # Base path
_FILE_FORMATS = ('.pdb', '.cif','.pdbqt', '.ent')

def list_internal_pdb_ids(internal_category: str = "proteins") -> List[str]:
    """List the PDB IDs of the internal PDB files bundled with the package for a specific category.

    Reads the contents of the `moremi_biokit.test_pdb_targets.<category>` directory.

    Args:
        internal_category (str, optional): The category of PDBs to list (e.g., "proteins", "smiles").
    ...
    """
    pdb_ids: List[str] = []
    qualified_pkg_path = f"{_INTERNAL_PDB_BASE_PKG_PATH}.{internal_category}"
    try:
        # Access the package resource directory
        pkg_path = importlib.resources.files(qualified_pkg_path)
        if pkg_path.is_dir():
            for item in pkg_path.iterdir():
                # Check if it's a file and ends with .pdb (case-insensitive)
                if item.is_file() and item.name.lower().endswith(_FILE_FORMATS):
                    pdb_ids.append(item.stem) # item.stem gets filename without extension
    except ModuleNotFoundError:
        print(f"Warning: Internal PDB directory for category '{internal_category}' ('{qualified_pkg_path}') not found.")
    except Exception as e:
        print(f"Error listing internal PDB files for category '{internal_category}': {e}")
    return sorted(pdb_ids)

def get_internal_pdb_path(pdb_id: str, internal_category: str = "proteins") -> Optional[str]:
    """Get the filesystem path to an internal PDB file from a specific category.
    ...
    Args:
        ...
        internal_category (str, optional): The category of PDBs (e.g., "proteins", "smiles").
                                           Defaults to "proteins".
    ...
    try:
        ...
        if pdb_resource.is_file():
            ...
            for item in pkg_path.iterdir():
                if item.is_file() and item.stem.lower() == pdb_id_lower and item.name.lower().endswith(_FILE_FORMATS):
                    return str(item.resolve())
            print(f"Warning: Internal PDB file for ID '{pdb_id}' (category: {internal_category}) not found in {qualified_pkg_path}.")
            return None
    except ModuleNotFoundError:
        print(f"Warning: Internal PDB directory for category '{internal_category}' ('{qualified_pkg_path}') not found.")
        return None
    except Exception as e:
        print(f"Error getting internal PDB path for '{pdb_id}' (category: {internal_category}): {e}")
        return None

def read_internal_pdb(pdb_id: str, internal_category: str = "proteins") -> Optional[str]:
    """Read the content of an internal PDB file from a specific category.
    ...
    try:
        ...
        print(f"Error reading internal PDB content for '{pdb_id}' (category: {internal_category}): {e}")
        return None
...
def _handle_local_pdb_file(file_path: str) -> Dict[str, Any]:
    ...
    if not file_path.lower().endswith(_FILE_FORMATS):
        print(f"Warning: File '{file_path}' does not have a typical PDB extension (.pdb, .ent, .cif, .pdbqt). Attempting to treat as PDB.")
...
def fetch_pdb(
    ...
    internal_pdb_category: str = "proteins" # New parameter for internal PDB category
) -> Dict[str, Any]:
        ...
        if use_internal_db:
            ...
            try:
                if source_db.lower() == 'rcsb':
                    ...
                    file_path = download_pdb_from_ncbi(identifier_str, output_dir) # Placeholder
                elif source_db.lower() == 'uniprot':
                    file_path = download_pdb_from_uniprot(identifier_str, output_dir) # Placeholder
                else:
